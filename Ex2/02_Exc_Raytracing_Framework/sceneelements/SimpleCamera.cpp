...
/*
  ...
  double pixel_length= 0.01;
  Ray r;
  r.point = Vector3(0.0,0.0,0.0);
  r.point.x = -pixel_length*(sample.getPosX() + sample.getOffset().x - 0.5*(m_resolutionX-1.0));
  r.point.y = pixel_length*(sample.getPosY() + sample.getOffset().y - 0.5*(m_resolutionY-1.0));
...
*/
  ...
  //=== EXERCISE 1.2.1 ===
  double pixel_length= (2.f/(tan(0.5f*PI - (m_openingAngle*PI/180.0))))/(m_resolutionY);

  Ray r;
  double dir_x=sample.getPosX()+sample.getOffset().x-0.5*(m_resolutionX-1);
  double dir_y=sample.getPosY()+sample.getOffset().y-0.5*(m_resolutionY-1);
  r.direction=m_camToWorld*Vector3(dir_x*pixel_length,dir_y*pixel_length,1.0);

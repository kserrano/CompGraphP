...
public:
	...
	static bool read(const std::string& _filenameOBJ, Mesh& _mesh, const std::string& _filenameMTL = std::string())
	{
...
private:
	...
	static bool parseOBJFile(const std::string& _filenameOBJ, Mesh& _mesh, const MaterialManager& _materialManager)
	{
        ...
        if(found1 == std::string::npos)
				{
					...
					int n = sscanf_s(buffer.c_str(), "%*s %d %d %d %d", &id1, &id2, &id3, &id4);
					if(n == 4) quad = true;
				}
				else 
				{
